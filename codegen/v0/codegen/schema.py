...
from codegen.spec import camel_to_snake, pascalize_schema_name, strip_product_prefix

SUCCESS_CODES = frozenset({"200", "201", "202", "207"})
...
def python_name_for(openapi_name: str, role: SchemaRole, shared_entities: set[str]) -> str:
    name = pascalize_schema_name(openapi_name)
    ...
    if role == SchemaRole.MUTATION_RESULT:
        if name.endswith("Result"):
            return name
        stem = name[: -len("Response")] if name.endswith("Response") else name
