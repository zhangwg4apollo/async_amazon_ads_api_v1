...
from codegen.spec import camel_to_snake, snake_to_pascal
...
def _render_import_header(tag: str, imports: ImportSet, shared_module: str | None, shared_names: list[str]) -> str:
    lines = [
        f'"""Auto-generated models for {tag} from Amazon Ads API v1."""',
        "",
        "from __future__ import annotations",
        "",
    ]
    ...
    if shared_module and shared_names:
        lines.append(f"from ads_api.models.v1._shared.{shared_module} import (")
...
def render_shared_module(product_module: str, emitted: list[EmittedModel], name_map: NameMap) -> str:
    return render_models_module(
        ...
        f'"""Auto-generated models for {product_module} (shared) from Amazon Ads API v1."""',
        f'"""Shared {product_module} models reused across entities."""',
    )
...
def _append_method(
    ...
    *,
    ...
    if is_array_resp and resp_model:
        ...
        lines.append(f"        return self._response({model_ret}, resp, mode=mode)")
    lines.append("")
    lines.append("")
