...
从 `codegen/v1/data/openapi/` 的 Merged OpenAPI 规范文件自动生成 `src/ads_api/client/v1` 与 `src/ads_api/models/v1`。
...
```bash
# 1. 下载 OpenAPI 规范（7 个 Merged + Reporting API beta Contract）
uv run python codegen/v1/download_openapi.py

# 2. 自动生成 v1 代码
uv run python codegen/v1/generate.py

# 可选：下载 Reporting API (beta) markdown 指南到 data/guides/
...
| `AmazonAdsAPIALLReportsContract_prod_3p_BETA.json` | Reporting (beta) | `general`（models）/ 顶层 `reports` | 无 | 无 |
...
- **客户端挂载方式**：
  ...
  - 无产品归属的通用接口直接挂在 `AdsClientV1` 顶层，如 `ads.v1.selling_accounts`、`ads.v1.brand_stores`、`ads.v1.reports`。
